...
namespace PlayWay.Water
{
	public class SpectrumResolverCPU
	{
		...
		private void InterpolationParams(float x, float z, int scaleIndex, float tileSize, out float fx, out float invFx, out float fy, out float invFy, out int index0, out int index1, out int index2, out int index3)
		{
			...
			fx = x * multiplier;
			fy = z * multiplier;
			int indexX = Mathf.FloorToInt(fx);
			int indexY = Mathf.FloorToInt(fy);
			fx -= indexX;
			fy -= indexY;
...
#if WATER_SIMD
			return new Vector4(result.X * scale, result.Y * 0.5f, result.Z * scale, result.W);
#else
			result.x = result.x * scale;
			result.z = result.z * scale;
			result.y *= 0.5f;
...
#region WavesSelecting
		...
		public GerstnerWave[] SelectShorelineWaves(int count)
		{
			...
			foreach(var spectrum in spectraDataList)
			{
				...
				lock (this)
				{
					...
					int numWaves = Mathf.Min(shorelineCandidates.Length, count);
		...
		virtual protected void InvalidateDirectionalSpectrum()
		{
			...
			for(int scaleIndex = 0; scaleIndex < numTiles; ++scaleIndex)
				tileSpectra[scaleIndex].directionalSpectrumDirty = true;
		}
