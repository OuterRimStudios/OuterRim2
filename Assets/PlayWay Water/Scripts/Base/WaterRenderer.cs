...
namespace PlayWay.Water
{
	...
	public class WaterRenderer
	{
		...
		public void Render(Camera camera, WaterGeometryType geometryType)
		{
			...
			if((!water.Volume.Boundless && water.Volume.HasAdditiveVolumes) && ((object)waterCamera == null || !waterCamera.RenderVolumes))
				return;

			MaterialPropertyBlock propertyBlock;

			if((object)waterCamera == null || !waterCamera.IsEffectCamera)
			{
			...
			{
				Graphics.DrawMesh(meshes[i], matrix, water.WaterMaterial, water.gameObject.layer, camera, 0, propertyBlock, water.ShadowCastingMode, water.ReceiveShadows, reflectionProbeAnchor == null ? water.transform : reflectionProbeAnchor);

				if((object)waterCamera == null || (waterCamera.ContainingWater != null && !waterCamera.IsEffectCamera))
					Graphics.DrawMesh(meshes[i], matrix, water.WaterBackMaterial, water.gameObject.layer, camera, 0, propertyBlock, water.ShadowCastingMode, water.ReceiveShadows, reflectionProbeAnchor == null ? water.transform : reflectionProbeAnchor);
			}
		}
		...
		public void OnSharedSubtractiveMaskRender(ref bool hasSubtractiveVolumes, ref bool hasBoundingVolumes, ref bool hasFlatMasks)
		{
			...
			if(useSharedMask)
			{
				...
				hasBoundingVolumes = hasBoundingVolumes || numBoundingVolumes != 0;
		...
		private void OnSomeCameraPreCull(Camera camera)
		{
			...
			{
				var waterCamera = WaterCamera.GetWaterCamera(camera);
				Render(camera, (object)waterCamera != null ? waterCamera.GeometryType : WaterGeometryType.Auto);
			}
		}
